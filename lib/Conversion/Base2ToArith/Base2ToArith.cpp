...
void ConvertBase2ToArithPass::runOnOperation()
{
    ...
    // Convert special IEEE-754 types to built-in floats.
    converter.addConversion([](IEEE754LikeType ieee754Ty) -> Type {
        const auto sema = ieee754Ty.getSemantics();
        if (const auto floatTy = sema.getBuiltinType()) return floatTy;
        return ieee754Ty;
    });
    ...
    converter.addSourceMaterialization(
        [](OpBuilder &builder, Type resultTy, ValueRange inputs, Location loc)
            -> Optional<Value> {
            if (getElementTypeOrSelf(resultTy)
                    .isa<FixedPointLikeType, IEEE754LikeType>()) {
                assert(inputs.size() == 1);
                return builder.create<BitCastOp>(loc, resultTy, inputs.front())
                    .getResult();
            }
