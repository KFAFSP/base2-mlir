...
//===----------------------------------------------------------------------===//
// Canonicalization (#bit.* -> builtin)
//===----------------------------------------------------------------------===//

// CHECK-LABEL: func.func @builtin_bits_i16(
func.func @builtin_bits_i16() -> (i16, i16, i16, i16, i16) {
    // CHECK-DAG: %[[RET:.+]] = bit.constant 16 : i16
    %0 = bit.constant #bit.bits<"0b0000000000010000"> : i16
    %1 = bit.constant #bit.bits<"0b10000/16"> : i16
    %2 = bit.constant #bit.bits<"0b10000000000010000/16"> : i16
    %3 = bit.constant #bit.bits<"0x10/16"> : i16
    %4 = bit.constant #bit.bits<"0x10010/16"> : i16
...
func.func @builtin_bits_f32() -> (f32, f32) {
    // CHECK-DAG: %[[RET:.+]] = bit.constant 1.342000e+01 : f32
    %0 = bit.constant #bit.bits<"0b_0_10000010_10101101011100001010010/32"> : f32
    %1 = bit.constant #bit.bits<"0x4156b852/32"> : f32
...
func.func @builtin_dense_bits_i1() -> (tensor<3xi1>, tensor<3xi1>, tensor<10xi1>) {
    // CHECK-DAG: %[[RET0:.+]] = bit.constant dense<true> : tensor<3xi1>
    %0 = bit.constant #bit.dense_bits<tensor<3xi1> = dense<true>>
    // CHECK-DAG: %[[RET1:.+]] = bit.constant dense<[false, true, true]> : tensor<3xi1>
    %1 = bit.constant #bit.dense_bits<tensor<3xi1> = dense<[false, true, true]>>
    // CHECK-DAG: %[[RET2:.+]] = bit.constant dense<[false, true, false, true, false, true, false, true, false, true]> : tensor<10xi1>
    %2 = bit.constant #bit.dense_bits<tensor<10xi1> = dense<"0xAA02">>
...
func.func @builtin_dense_bits_i16() -> (tensor<3xi16>, tensor<3xi16>, tensor<3xi16>) {
    // CHECK-DAG: %[[RET0:.+]] = bit.constant dense<1> : tensor<3xi16>
    %0 = bit.constant #bit.dense_bits<tensor<3xi16> = dense<1>>
    // CHECK-DAG: %[[RET1:.+]] = bit.constant dense<[1, 2, 3]> : tensor<3xi16>
    %1 = bit.constant #bit.dense_bits<tensor<3xi16> = dense<[1, 2, 3]>>
    %2 = bit.constant #bit.dense_bits<tensor<3xi16> = dense<"0x010002000300">>
...
func.func @builtin_dense_bits_f16() -> (tensor<3xf16>, tensor<3xf16>, tensor<3xf16>) {
    // CHECK-DAG: %[[RET0:.+]] = bit.constant dense<1.342190e+01> : tensor<3xf16>
    %0 = bit.constant #bit.dense_bits<tensor<3xf16> = dense<"0x4AB6">>
    // CHECK-DAG: %[[RET1:.+]] = bit.constant dense<[1.342190e+01, 1.432030e+01, 4.321880e+01]> : tensor<3xf16>
    %1 = bit.constant #bit.dense_bits<tensor<3xf16> = dense<[0xB64A, 0x294B, 0x6751]>>
    %2 = bit.constant #bit.dense_bits<tensor<3xf16> = dense<"0x4AB64B295167">>
    // CHECK: return %[[RET0]], %[[RET1]], %[[RET1]]
