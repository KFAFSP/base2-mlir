...
func.func @value_cast_ui_si_of() -> (si8, si8, si8, si8, si8) {
    ...
    %0 = base2.cast %cst : ui8 to si8
    %1 = base2.cast %cst : ui8 to nearest si8
    %2 = base2.cast %cst : ui8 to round_down si8
    %3 = base2.cast %cst : ui8 to towards_zero si8
    %4 = base2.cast %cst : ui8 to away_from_zero si8
...
func.func @value_cast_si_ui_uf() -> (ui8, ui8, ui8, ui8, ui8) {
    ...
    %0 = base2.cast %cst : si8 to ui8
    %1 = base2.cast %cst : si8 to nearest ui8
    %2 = base2.cast %cst : si8 to round_up ui8
    %3 = base2.cast %cst : si8 to towards_zero ui8
    %4 = base2.cast %cst : si8 to away_from_zero ui8
...
func.func @value_cast_ui_ui_of() -> (ui4, ui4, ui4, ui4, ui4) {
    ...
    %0 = base2.cast %cst : ui8 to ui4
    %1 = base2.cast %cst : ui8 to nearest ui4
    %2 = base2.cast %cst : ui8 to round_down ui4
    %3 = base2.cast %cst : ui8 to towards_zero ui4
    %4 = base2.cast %cst : ui8 to away_from_zero ui4
    // CHECK: return %[[MOD]], %[[SAT]], %[[SAT]], %[[MOD]], %[[SAT]]
...
func.func @value_cast_si_si_of() -> (si4, si4, si4, si4, si4) {
    ...
    %0 = base2.cast %cst : si8 to si4
    %1 = base2.cast %cst : si8 to nearest si4
    %2 = base2.cast %cst : si8 to round_down si4
    %3 = base2.cast %cst : si8 to towards_zero si4
    %4 = base2.cast %cst : si8 to away_from_zero si4
    // CHECK: return %[[MOD]], %[[SAT]], %[[SAT]], %[[MOD]], %[[SAT]]
...
func.func @value_cast_si_si_uf() -> (si4, si4, si4, si4, si4) {
    ...
    %0 = base2.cast %cst : si8 to si4
    %1 = base2.cast %cst : si8 to nearest si4
    %2 = base2.cast %cst : si8 to round_up si4
    %3 = base2.cast %cst : si8 to towards_zero si4
    %4 = base2.cast %cst : si8 to away_from_zero si4
    // CHECK: return %[[MOD]], %[[SAT]], %[[SAT]], %[[SAT]], %[[MOD]]
    return %0, %1, %2, %3, %4 : si4, si4, si4, si4, si4
...
func.func @cmp_ui() -> (i1, i1, i1, i1, i1, i1, i1) {
    ...
    %lhs = bit.constant 4 : ui8
    %rhs = bit.constant 8 : ui8
...
func.func @cmp_si() -> (i1, i1, i1, i1, i1, i1, i1) {
    // CHECK-DAG: %[[FALSE:.+]] = bit.constant false
    // CHECK-DAG: %[[TRUE:.+]] = bit.constant true
    %lhs = bit.constant -8 : si8
    %rhs = bit.constant 4 : si8
...
func.func @add_ui_of() -> (ui4, ui4, ui4, ui4, ui4) {
    // CHECK-DAG: %[[MOD:.+]] = bit.constant 1 : ui4
    // CHECK-DAG: %[[SAT:.+]] = bit.constant 15 : ui4
    %lhs = bit.constant 7 : ui4
    %rhs = bit.constant 10 : ui4
...
func.func @add_si_of() -> (si4, si4, si4, si4, si4) {
    // CHECK-DAG: %[[MOD:.+]] = bit.constant -5 : si4
    // CHECK-DAG: %[[SAT:.+]] = bit.constant 7 : si4
    %lhs = bit.constant 7 : si4
    %rhs = bit.constant 4 : si4
...
func.func @add_si_uf() -> (si4, si4, si4, si4, si4) {
    // CHECK-DAG: %[[MOD:.+]] = bit.constant 7 : si4
    // CHECK-DAG: %[[SAT:.+]] = bit.constant -8 : si4
    %lhs = bit.constant -4 : si4
    %rhs = bit.constant -5 : si4
    %0 = base2.add %lhs, %rhs : si4
    %1 = base2.add %lhs, %rhs : nearest si4
...
func.func @sub_ui_uf() -> (ui4, ui4, ui4, ui4, ui4) {
    // CHECK-DAG: %[[MOD:.+]] = bit.constant 13 : ui4
    // CHECK-DAG: %[[SAT:.+]] = bit.constant 0 : ui4
    %lhs = bit.constant 3 : ui4
    %rhs = bit.constant 6 : ui4
...
func.func @sub_si_uf() -> (si4, si4, si4, si4, si4) {
    // CHECK-DAG: %[[MOD:.+]] = bit.constant 7 : si4
    // CHECK-DAG: %[[SAT:.+]] = bit.constant -8 : si4
    %lhs = bit.constant -3 : si4
    %rhs = bit.constant 6 : si4
    %0 = base2.sub %lhs, %rhs : si4
    %1 = base2.sub %lhs, %rhs : nearest si4
...
func.func @mul_ui_of() -> (ui4, ui4, ui4, ui4, ui4) {
    // CHECK-DAG: %[[MOD:.+]] = bit.constant 2 : ui4
    // CHECK-DAG: %[[SAT:.+]] = bit.constant 15 : ui4
    %lhs = bit.constant 3 : ui4
    %rhs = bit.constant 6 : ui4
...
func.func @div_si_of() -> (si4, si4, si4, si4, si4) {
    // CHECK-DAG: %[[MOD:.+]] = bit.constant -8 : si4
    // CHECK-DAG: %[[SAT:.+]] = bit.constant 7 : si4
    %lhs = bit.constant -8 : si4
    %rhs = bit.constant -1 : si4
...
func.func @mod_si_sign() -> (si8, si8, si8, si8) {
    // CHECK-DAG: %[[REM_POS:.+]] = bit.constant 1 : si8
    // CHECK-DAG: %[[REM_NEG:.+]] = bit.constant -1 : si8
    %lhs_pos = bit.constant 10 : si8
    %lhs_neg = bit.constant -10 : si8
    %rhs_pos = bit.constant 3 : si8
    %rhs_neg = bit.constant -3 : si8
...
func.func @mod_si_of() -> (si4) {
    // CHECK-DAG: %[[REM:.+]] = bit.constant 0 : si4
    %lhs = bit.constant -8 : si4
    %rhs = bit.constant -1 : si4
